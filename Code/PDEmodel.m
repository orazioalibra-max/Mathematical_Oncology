...
function PDEmodel()
    ...
    %First Block: PDE
    Apde = spdiags(ones(r,1)*[v_y,-v_y]/h,-1:0,r,r)- spdiags(delta_y, 0, r, r);
    %Apde(r,r) = Apde(r,r)+v_y/h; % boundary condition v(t,0)=0
    ...
    function dxdt = ODE(~, x)
        ...
        integral = hphy*(w.*lambda_y')*x(5:end); %trapezoidal rule
