...
lambdas=[lambda_E,lambda_E,lambda_E];
deltas=[delta_E,delta_E,delta_E];
r_t=5*rE; %fianda

odefun2= @(t,x) [alpha*x(1)*(1-(x(1)/Tc))-h*((lambda_K*x(1)*x(2)+lambdas*x(1)*[x(4);x(6);x(7)])/(x(1)+x(2)+x(4)+x(6)+x(7)+1));...
    h*x(5)*x(2)-delta_K*x(2);...
    2^m*sB*x(1)+h*x(5)*x(3)-rE*x(3)-delta_B*x(3);...
    rE*x(3)-r_t*x(4)-deltas(1)*x(4);...
    rho_K*x(2)+rho_B*x(3)-h*x(5)*(x(2)+x(3))-delta_P*x(5);...
    r_t*x(4)-r_t*x(6)-deltas(2)*x(6);...
    r_t*x(6)-deltas(3)*x(7)];

x0_E=[1,2*1e-4,0,0,0,0,0];
...
lambdas=[lambda_K,lambda_K+0.5*(lambda_E-lambda_K),lambda_E];
deltas=[delta_K,delta_K+0.5*(delta_E-delta_K),delta_E];
r_t=5*rE; %FIANDA
